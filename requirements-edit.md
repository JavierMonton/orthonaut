I want to add new features to this application. 

It works great, when the system detects a wrong word, it shows the list of words, with a button to mark them as "valid word"

I want to add a new button on each word with a new feature. 

The button will say "expand", and it will expand the row to show below a block of text coming from the website, for example, the paragraph where the word is. 
Because the word can appear in multiple places, by default only the first paragraph will be shown, but there will be a "next" button to show the next paragraph where the word appears.
